...
const TimeBlock = ({ value, label }: { value: number, label: string }) => (
    <div className="flex flex-col items-center justify-center bg-black/25 backdrop-blur-sm rounded-lg py-2 px-1 w-full border border-line aspect-square max-h-[80px]">
        <span className="font-black font-mono tracking-tighter text-4xl" style={{ fontVariantNumeric: 'tabular-nums' }}>
            {value.toString().padStart(2, '0')}
        </span>
        <span className="uppercase font-bold tracking-widest text-ink-muted mt-1 text-t-2xs">{label}</span>
    </div>
);
...
export function LiveCountdown({ targetDate, targetTime = '00:00' }: { targetDate: string, targetTime?: string }) {
    ...
    return (
        <div className="grid grid-cols-4 mt-4 w-full max-w-[400px] mx-auto gap-2">
...
`}
        ...
        >
            ...
            <div className="absolute inset-0 bg-white/0 group-hover:bg-white/5 transition-colors items-center justify-center opacity-0 group-hover:opacity-100 z-50 flex">
            ...
            <div className="relative z-10 flex flex-col items-center w-full">
                <div className="flex items-center justify-center gap-2 text-white/80 font-bold uppercase tracking-widest mb-3 text-xs">
                    <Trophy size={mobile ? 14 : 16} /> TU PRÓXIMO RETO
                </div>
                <h3 className={`
 ${activeFont === 'bebas' ? 'text-5xl' : 'text-3xl'}
...
`}>
            ...
            <div className="relative z-10 mt-4 w-full max-w-xl shrink-0 bg-black/30 backdrop-blur-md pb-4 pt-0 rounded-3xl border border-line flex justify-center shadow-inner px-6">
