...
export function CountdownWidget({ assigned, userId }: CountdownWidgetProps) {
    ...
    if (!event) {
        return (
            <div className="relative flex h-full min-h-[160px] flex-col items-center justify-center overflow-hidden rounded-card border border-[var(--border-default)] bg-surface-raised text-center p-6">
    ...
    return (
        <div className={`${colorClass} relative flex h-full min-h-[160px] flex-col items-center justify-center overflow-hidden rounded-card text-center text-ink p-6`}>
            ...
            <div className="relative z-10 flex flex-col items-center w-full">
                <div className="flex items-center justify-center gap-2 text-white/80 font-bold uppercase tracking-widest mb-2 text-xs">
                    <Trophy size={14} /> {prefs.source === 'custom' ? 'TU OBJETIVO' : prefs.source === 'aep' ? 'CALENDARIO AEP' : 'TU PRÓXIMO RETO'}
                </div>
                <h3 className="mb-2 font-black uppercase italic drop-shadow-lg text-t-2xl">
...
function CountdownSettings({
    ...
    return (
        <AnimatePresence>
            <div className="fixed inset-0 z-[200] flex justify-center items-center p-4" onClick={(e) => e.stopPropagation()}>
                <m.div
                ...
                <m.div
                    ...
                    exit={{ opacity: 0, y: 40 }}
                    className="relative bg-surface-canvas w-full border border-line shadow-2xl flex flex-col max-h-[85vh] text-left max-w-lg rounded-2xl"
                >
                    {/* Header */}
