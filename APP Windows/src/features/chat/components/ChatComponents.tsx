...
export const ChatBubble: React.FC<{ message: ChatMessage; isOwn: boolean }> = ({ message, isOwn }) => {
    return (
        ...
        >
            <div className={`p-4 rounded-[1.5rem] relative max-w-[70%] ${
 isOwn 
...
export const ChatInput: React.FC<{ onSend: (val: string) => void }> = ({ onSend }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 bg-black/40 backdrop-blur-xl border-t border-subtle flex items-center gap-3">
            <button type="button" className="p-3 text-ink-subtle hover:text-ink transition-colors">
                <ImageIcon size={20} />
            </button>
            <input 
                value={text}
