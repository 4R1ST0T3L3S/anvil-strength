    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            try {
                ...
                type Assignment = {
                    id: string; name: string; date: string; athlete_id: string; level?: string;
                    // Puede venir NULL: el perfil del atleta deja de ser legible
                    // en cuanto se rompe el vínculo con el coach, y la
                    // competición sigue siendo suya igualmente.
                    athlete?: { full_name: string; avatar_url?: string } | null;
                };

                const grouped = (assignments as Assignment[]).reduce((acc: Record<string, CompetitionGroup>, item: Assignment) => {
                ...
                {competitions.length === 0 ? (
                    ...
                    competitions.map((comp: CompetitionGroup, index: number) => (
                        <m.div 
                            ...
                            className="bg-surface-sunken rounded-xl overflow-hidden border border-subtle shadow-md flex relative flex-row"
                        >
                            <div className="p-4 flex-1 flex gap-6 pr-16 flex-row">
                                {/* Competition Info */}
                                ...
                                {/* Athletes List */}
                                <div className="flex-1 grid gap-2 border-subtle grid-cols-3 border-t-0 pt-0">
                            ...
                            {/* Side tag */}
                            <div className="absolute right-0 top-0 h-full w-14 bg-black/40 flex-col items-center justify-center border-l border-subtle flex">
