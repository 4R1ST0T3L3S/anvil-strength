...
export function VideoTracker({ onTrackingComplete, seekTime, isResultMode, onTimeUpdate }: VideoTrackerProps) {
    ...
    return (
        <div className={`flex flex-col gap-3 ${isResultMode ? 'h-full' : ''}`}>
            ...
            {state === 'upload' ? (
                <div
                    className="flex cursor-pointer flex-col items-center justify-center rounded-sheet border-2 border-dashed border-subtle text-center transition-colors duration-fast hover:border-brand/50 p-16"
                    onClick={() => document.getElementById('video-upload')?.click()}
                >
                    <Upload size={40} className="mb-3 text-ink-faint" />
                    <h3 className="mb-1 font-bold text-ink text-t-lg">Selecciona un vídeo</h3>
                ...
                <>
                    <div className={`relative flex items-center justify-center bg-black ${isResultMode ? 'h-full w-full flex-1 overflow-hidden rounded-sheet border border-subtle' : 'aspect-video overflow-hidden rounded-sheet border border-subtle max-h-[70vh]'}`}>
