...
export function TeamCard({
    ...
    return (
        <button
            onClick={onClick}
            className="group relative flex min-w-0 flex-col justify-between gap-4 overflow-hidden rounded-card bg-brand text-left transition-colors duration-fast ease-snap hover:bg-brand-hover active:bg-brand-active min-h-[132px] p-5"
        >
            {/* MARCA DE AGUA.
            ...
            <Users
                size={104}
                aria-hidden="true"
                className="pointer-events-none absolute -top-4 text-brand-ink opacity-[0.12] transition-transform duration-base ease-snap group-hover:scale-105 -right-6 h-32 w-32"
            />
            ...
            <span className="relative min-w-0">
                ...
                <span className="block text-balance font-black uppercase tracking-display text-brand-ink text-t-2xl">
                    Mis atletas
                </span>
...
export function DietsCard({
    ...
    return (
        <button
            onClick={onClick}
            className="group relative flex min-w-0 flex-col justify-between gap-4 overflow-hidden rounded-card border border-[var(--border-default)] bg-surface-raised text-left transition-colors duration-fast ease-snap hover:border-[var(--border-strong)] hover:bg-surface-overlay min-h-[132px] p-5"
        >
            <Apple
                size={104}
                aria-hidden="true"
                className="pointer-events-none absolute -top-4 text-ink opacity-[0.05] transition-transform duration-base ease-snap group-hover:scale-105 -right-6 h-32 w-32"
            />
            <Apple size={24} className="relative shrink-0 text-brand-text" aria-hidden="true" />
            <span className="relative min-w-0">
                <span className="block text-balance font-black uppercase tracking-display text-ink text-t-2xl">
                    Dietas
                </span>
    ...
    return (
        <button
            ...
            className={`group flex min-w-0 items-center gap-4 rounded-card border border-[var(--border-default)] bg-surface-raised text-left transition-colors duration-fast ease-snap hover:border-[var(--border-strong)] hover:bg-surface-overlay ${compact
                ? 'min-h-[96px] p-4'
                : 'min-h-[132px] gap-5 p-5'
                }`}
        >
            <span className="shrink-0 text-center">
                <span className={`block font-black leading-none tabular-nums text-brand-text ${compact ? 'text-t-2xl' : 'text-metric'
                    }`}>
                    {days}
                </span>
            ...
            <span className="min-w-0 flex-1">
                <span className="block truncate font-bold text-ink text-t-base">
                    {comp.name}
                </span>
...
export function NoCompCard({ compact = false }: { compact?: boolean }) {
    return (
        <div className={`flex min-w-0 items-center gap-4 rounded-card border border-dashed border-[var(--border-default)] ${compact ? 'min-h-[96px] p-4' : 'min-h-[132px] p-5'
            }`}>
...
export function LessonsCard({ quote }: { quote: string }) {
    return (
        <div className="relative flex min-h-[96px] min-w-0 flex-col justify-center overflow-hidden rounded-card border border-[var(--border-default)] bg-surface-raised p-4">
            ...
            />
            <p className="relative text-balance font-black uppercase tracking-display text-ink text-t-lg">
                {quote}
            </p>
