            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="font-black uppercase tracking-tighter text-ink text-2xl">
                            Calendario AEP 2026
                        </h2>
                    </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                    {competitions.map((comp, index) => {
                        ...
                        return (
                            <div key={index} className={`relative bg-surface-sunken border-y border-r ${border} border-l-[6px] ${line} rounded-[1.5rem] transition-[background-color,border-color,opacity] hover:bg-surface-raised hover:border-r-white/10 p-5 ${esInvitado ? 'opacity-70' : ''}`}>
                                <div className="flex flex-col gap-4">

                                    {/* SECCIÓN SUPERIOR: Badges y Fecha */}
                                    <div className="flex justify-between items-start">
                                        <div className="flex flex-col gap-1.5">
                                            <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-black uppercase tracking-widest text-xs ${bg} ${color}`}>
                                                {icon && icon}
                                                {level}
                                            </div>
                                            {esInvitado && (
                                                <div className={`flex items-center gap-1.5 ${bg} ${color} text-t-2xs font-black px-2.5 py-1 rounded-lg uppercase border border-subtle`}>
                                        ...
                                        <div className="flex items-center gap-2 text-ink font-black uppercase tracking-widest bg-white/5 border border-subtle px-3 py-1.5 rounded-xl shrink-0 shadow-inner text-t-xs">
                                            <CalendarIcon size={14} className={color} />
                                    ...
                                    <div className="mt-0.5">
                                        <h3 className="font-black text-ink uppercase tracking-tighter italic text-2xl">
                                            {comp.campeonato}
                                        </h3>
                                    </div>
                                    ...
                                    <div className="flex justify-between flex-row items-center gap-0">
                                        <div className="flex items-center gap-2 text-ink font-black uppercase tracking-widest text-sm">
                                            <div className={`p-1.5 rounded-lg ${bg}`}>
                                                <MapPin size={16} className={color} />
                                            </div>
                                            <span className="leading-tight">{comp.sede}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 self-auto">
                                            {user?.role === 'coach' && (
                                                <button
                                                    ...
                                                    className="px-6 py-2 bg-brand text-ink text-t-xs font-black uppercase tracking-widest rounded-xl hover:bg-red-600 transition-[background-color,transform] shadow-lg active:scale-95"
                                                >
                                                    Asignar
                                                </button>
                                            )}
                                            {user?.role === 'athlete' && (
                                                <button
                                                    ...
                                                    className="px-6 py-2 bg-white text-black text-t-xs font-black uppercase tracking-widest rounded-xl hover:bg-gray-200 transition-[background-color,opacity,transform] active:scale-95 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                                                >
