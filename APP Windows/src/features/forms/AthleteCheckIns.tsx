...
export function CheckInCard({ athleteId }: { athleteId: string }) {
    ...
    return (
        <div className="flex flex-col">
            <SectionLabel icon={ClipboardCheck} colorClass={SECTION.checkin}>Check-in</SectionLabel>
                ...
                {(['daily', 'weekly'] as FormType[]).map(type => {
                    ...
                    return (
                        <NavTile
                            key={type}
                            icon={CalendarCheck}
                            title={type === 'daily' ? 'Diario' : 'Semanal'}
                            hint={done ? 'Hecho · editar respuesta' : type === 'daily' ? '¿Cómo fue la sesión?' : 'Resumen de la semana'}
                            onClick={() => setOpenForm(type)}
                            customColor={done ? TILE.success : TILE.brand}
                        />
                    );
                })}
            </div>
