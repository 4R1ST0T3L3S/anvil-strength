...
export function DayEditorModal({
    ...
    /**
     * Elegir ejercicio: lo abre en el panel central.
     */
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-surface-canvas flex flex-col animate-fade">
            {/* Header */}
            <div className="flex items-center justify-between gap-4 py-4 border-b border-subtle bg-surface-canvas shrink-0 px-8">
                <div className="flex items-center gap-4 min-w-0 flex-1">
                    ...
                    <input
                        className="bg-transparent font-black text-ink w-full placeholder-gray-600 uppercase tracking-tight border-b-2 border-transparent focus:border-brand/50 transition-colors min-w-0 text-2xl"
                        value={session.name ?? ''}
                ...
                <div className="flex items-center gap-2 shrink-0 relative">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`items-center gap-1.5 px-3 py-2.5 rounded-xl text-t-2xs font-black uppercase transition-colors border flex ${openMenu === 'copy' ? 'bg-brand/10 border-brand/40 text-brand-text' : 'bg-white/5 border-[var(--border-default)] text-ink-muted hover:text-ink'}`}
                    ...
                    <div className="relative">
                        ...
                        >
                            <LayoutTemplate size={14} /> <span className="inline">Plantillas</span>
                        </button>
                        {openMenu === 'templates' && (
                            <div className="absolute top-full mt-2 z-40 bg-surface-raised border border-[var(--border-default)] rounded-xl shadow-2xl p-3 left-auto right-0 w-72 mx-0">
                    ...
                    >
                        <Eye size={14} /> <span className="inline">Vista atleta</span>
                    </button>
            ...
            {session.exercises.length === 0 ? (
                ...
                <div className="flex flex-1 min-h-0 w-full">
                    ...
                    <div className="flex shrink-0 flex-col gap-2 overflow-y-auto border-subtle bg-surface-canvas p-3 scrollbar-hide min-h-0 border-r w-96">
                    ...
                    <div className="overflow-y-auto min-h-0 p-6 w-auto flex-1 shrink">
                    ...
                    {/* DERECHA: resumen de métricas del día.
                        ...
                        en 288px, que con las tarjetas de volumen y el desglose
                        por músculo obligaba a leer las cifras en columnas de
                        dos dígitos. */}
                    <div
                        style={{ '--panel-w': `${panel.width}px` } as React.CSSProperties}
                        className="shrink-0 space-y-4 overflow-y-auto border-subtle bg-surface-canvas p-4 min-h-0 w-[var(--panel-w)] border-l"
                    >
