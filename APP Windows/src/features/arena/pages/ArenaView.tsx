...
export function ArenaView({ user }: { user: ExtendedProfile }) {
    ...
    return (
        <div className="min-h-[100dvh] bg-surface-sunken text-ink">
            {/* Header */}
            <header className="fixed top-0 left-0 right-0 z-50 bg-black/60 backdrop-blur-xl border-b border-subtle py-4 flex items-center justify-between px-6">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-brand rounded-lg text-black shadow-[0_0_15px_rgba(220,38,38,0.4)]">
                        <Swords size={18} />
                    </div>
                    <div>
                        <h1 className="font-black uppercase italic tracking-tighter text-xl">La Arena</h1>
            ...
            <main className="pt-24 pb-12 w-full px-12">
 ...
 activeTab === 'history' ? 'bg-brand text-ink shadow-lg shadow-red-900/20' : 'text-ink-subtle hover:text-ink'
                    ...
                    {/* Sidebar */}
                    <div className="space-y-6 w-full pr-0">
