...
import { Loader, Save, Camera, Trash2, CheckCircle2, AlertCircle, LogOut, FileText, ChevronRight, Dumbbell, Users } from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';
import { ConfirmationModal } from '../../../components/modals/ConfirmationModal';
import { isStaff, tieneAmbosPaneles } from '../../../lib/roles';
...
export function ProfileSection({ user, onUpdate, onBack }: ProfileSectionProps) {
 ...
 : 'border-[var(--danger-line,var(--border-strong))] bg-[var(--danger-quiet)] text-danger-text'
            ...
            <form id="profile-form" onSubmit={handleSubmit} className="space-y-5">
                {/* Avatar */}
                <div className="flex items-center gap-6 rounded-card border border-[var(--border-default)] bg-surface-raised flex-row p-8">
                    <div className="relative shrink-0">
                        <div className="flex items-center justify-center overflow-hidden rounded-pill border-2 border-[var(--border-strong)] bg-surface-sunken h-32 w-32">
                            {imagePreview ? (
            ...
            <div className="mt-6 flex justify-end">
                <button
                    ...
                    form="profile-form"
                    disabled={isSaving}
                    className="flex items-center justify-center gap-2 rounded-field bg-brand px-8 py-3 text-t-sm font-extrabold uppercase tracking-wide text-brand-ink transition-colors duration-fast ease-snap hover:bg-brand-hover disabled:opacity-40 w-auto"
                >
            ...
            <div className="mt-8 border-t border-subtle pt-6">
                {/* CONMUTADOR DE PANEL. Quien entrena a gente y además le
                    entrenan cambia de panel desde aquí, como en la app
                    Android: el perfil es el único sitio que comparten los
                    dos paneles. */}
