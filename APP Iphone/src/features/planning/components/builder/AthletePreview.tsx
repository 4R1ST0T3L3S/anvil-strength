    ...
    return (
        ...
        >
            <m.div
                ...
                className="relative w-full max-w-[380px] h-[80vh] bg-surface-canvas rounded-[2.5rem] border-4 border-surface-overlay shadow-overlay overflow-hidden flex flex-col"
            >
                {/* Notch decorativo */}
                ...
                <div className="shrink-0 px-5 py-3 bg-surface-canvas border-b border-subtle flex items-center justify-between">
                    <div>
                        <p className="text-t-2xs font-semibold text-brand-text">Así lo verá el atleta</p>
                        <h3 className="font-semibold text-ink text-lg leading-tight">{session.name || `Día ${session.day_number}`}</h3>
                    </div>
                    <button onClick={onClose} className="p-2 bg-[var(--fill-muted)] hover:bg-[var(--fill-pressed)] rounded-full text-ink-muted hover:text-ink transition-colors">
                        <X size={16} />
                    </button>
                </div>
                        ...
                        session.exercises.map(ex => (
                            <div key={ex.id} className="bg-surface-canvas rounded-card overflow-hidden border border-subtle">
                                <div className="p-4 bg-surface-raised">
                                    <h4 className="font-bold text-base leading-tight text-ink">{ex.exercise?.name}</h4>
                                    ...
                                    {(ex.rpe || ex.velocity_avg || ex.rest_seconds) && (
                                        <div className="flex gap-3 mt-2 text-t-2xs font-bold text-ink-subtle">
                                    ...
                                    {ex.notes && <p className="text-xs text-ink-muted mt-2">"{ex.notes}"</p>}
                                </div>
                                <div className="p-3 space-y-1.5">
                                    {ex.sets.map((set, i) => (
                                        <div key={set.id} className="flex items-center justify-between bg-black/20 rounded-lg px-3 py-2 text-sm">
                                            <span className="text-t-2xs font-semibold text-ink-subtle">Serie {i + 1}</span>
                                            <span className="font-bold text-ink font-mono">
                                                {set.target_reps || '—'}{set.target_load ? ` @ ${set.target_load}kg` : ''}{set.target_rpe ? ` RPE ${set.target_rpe}` : ''}
                                            </span>
                                        </div>
                                    ))}
                                    {ex.sets.length === 0 && <p className="text-t-2xs text-ink-subtle px-1">Sin series prescritas</p>}
                                </div>
                            </div>
                        ))
                    )}
    ...
    return (
        <div className="overflow-hidden rounded-card border border-subtle bg-surface-canvas">
            <div className="border-l-2 border-brand px-3.5 py-3">
                <p className="text-t-2xs font-semibold text-brand-text">{label}</p>
