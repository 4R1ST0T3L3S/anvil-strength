# ⚒️ Anvil Strength App — variante PWA para iPhone

> Esta carpeta es la **app instalable para iPhone** (PWA desde Safari → "Añadir a
> pantalla de inicio"), servida en `https://ios.anvilstrength.es`. Nació como
> copia de `APP Android/` y se le quitó todo Capacitor: aquí no hay `android/`,
> ni `capacitor.config.ts`, ni push por Firebase — las notificaciones son Web
> Push (`public/push-sw.js`, requiere `VITE_VAPID_PUBLIC_KEY`). No se indexa
> (`noindex` + `robots.txt`): la web pública sigue siendo `anvilstrength.es`.
