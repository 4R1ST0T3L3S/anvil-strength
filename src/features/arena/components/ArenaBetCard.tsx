...
import { Swords, Users, HelpCircle, Trophy, TrendingUp } from 'lucide-react';
import { ArenaBet, ArenaOption } from '../../../types/database';

interface ArenaBetCardProps {
    bet: ArenaBet & { options: ArenaOption[] };
    onBetClick: (bet: ArenaBet, option: ArenaOption) => void;
}

export const ArenaBetCard: React.FC<ArenaBetCardProps> = ({ bet, onBetClick }) => {
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-6 left-6 z-10 flex items-center gap-2">
                <div className="p-2 bg-anvil-red/10 rounded-lg text-anvil-red border border-anvil-red/20 shadow-[0_0_15px_rgba(220,38,38,0.2)]">
                    {getIcon()}
                </div>
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/5">
                    {getTypeLabel()}
                </span>
            </div>
            ...
            <div className="p-6 md:p-8 pt-20">
                <h3 className="text-xl md:text-2xl font-black uppercase italic text-white tracking-tighter mb-2 group-hover:text-anvil-red transition-colors">
                    {bet.title}
                </h3>
                {bet.description && (
                    <p className="text-gray-500 text-[9px] md:text-xs font-bold uppercase tracking-widest mb-6">
                        {bet.description}
                    </p>
                )}
                ...
                {bet.type === 'prediction' ? (
                    <button
                        onClick={() => bet.status === 'open' && onBetClick(bet, null as any)}
                    ...
                    <div className={`grid gap-4 ${bet.type === '1vs1' ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-2'}`}>
                        {bet.options?.map((option, index) => {
                            ...
                            return (
                                <button
                                    key={option.id}
                                    onClick={() => bet.status === 'open' && onBetClick(bet, option)}
                                    disabled={bet.status !== 'open'}
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col items-center text-center gap-2">
                                        ...
                                        <span className="text-lg font-black text-white/40 group-hover/option:text-white transition-colors">
