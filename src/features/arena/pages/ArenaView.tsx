...
export function ArenaView({ user }: { user: ExtendedProfile }) {
    ...
    const handleCancelFight = async (fight: Fight) => {
        if (!window.confirm("¿Anular combate? Se devolverán los AC a todos los apostantes.")) return;
        setIsActionLoading(true);
        try {
            const { data: bets } = await supabase.from('arena_fight_bets').select('*').eq('fight_id', fight.id);
            if (bets && bets.length > 0) {
                // Agrupamos devoluciones por ID de usuario para evitar inconsistencias
    ...
    const handleResolveFight = async (winnerSide: 'a' | 'b') => {
        ...
        try {
            const { data: winningBets } = await supabase.from('arena_fight_bets').select('*').eq('fight_id', fight.id).eq('selected_athlete_id', winnerId);

            if (winningBets) {
                // Agrupamos premios por usuario (Fórmula Pari-mutuel proporcional)
            ...
            await supabase.from('arena_fights').update({ status: 'resolved', winner_id: winnerId }).eq('id', fight.id);
            setResolveModal({ open: false, fight: null });
            await fetchData();
            alert("Premios repartidos con éxito.");
        } catch (err) { console.error(err); } finally { setIsActionLoading(false); }
    };
    ...
    return (
        <div className="p-4 md:p-8 text-white min-h-screen animate-in fade-in duration-500">
            ...
            <div className="grid lg:grid-cols-2 gap-8 items-start">
                <div className="space-y-8">
                    ...
                    <div className="flex gap-2 p-1 bg-black/40 rounded-2xl border border-white/5">
                        <button onClick={() => setActiveTab('active')} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase italic transition-all flex items-center justify-center gap-2 ${activeTab === 'active' ? 'bg-yellow-500 text-black' : 'text-gray-500 hover:text-white'}`}>
                            <Swords size={16} /> En Vivo
                        </button>
                        <button onClick={() => setActiveTab('history')} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase italic transition-all flex items-center justify-center gap-2 ${activeTab === 'history' ? 'bg-yellow-500 text-black' : 'text-gray-500 hover:text-white'}`}>
                            <History size={16} /> Historial
                        </button>
                    </div>

                    {/* LISTADO DINÁMICO (ACTIVOS O HISTORIAL) */}
                    <div className="space-y-6">
                        {activeTab === 'active' ? (
                            activeFights.length > 0 ? activeFights.map(fight => {
                                ...
                                return (
                                    <div key={fight.id} className="bg-[#1a1a1a] border border-white/5 rounded-[2.5rem] p-8 shadow-xl border-t-red-500/20 border-t-4">
                                        <div className="flex justify-between items-center mb-6 text-[10px] font-black text-gray-500 tracking-widest uppercase italic">
                                            <span className="text-red-500 bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20 tracking-tighter">DUELO EN VIVO</span>
                                            {fight.title}
                                        </div>
                                        <div className="flex justify-between items-center gap-4 text-center">
                                            <div className="flex-1">
                                                ...
                                                <h4 className="font-black text-white uppercase italic text-xs truncate px-2">{fight.athlete_a?.full_name || "Atleta A"}</h4>
                                            ...
                                            <div className="flex-1">
                                                <div className="w-16 h-16 bg-gray-800 rounded-full mx-auto mb-2 border border-white/5 flex items-center justify-center overflow-hidden"><User size={30} className="text-gray-600" /></div>
                                                <h4 className="font-black text-white uppercase italic text-xs truncate px-2">{fight.athlete_b?.full_name || "Atleta B"}</h4>
                                        ...
                                        <div className="mt-8">
                                            ...
                                            <div className="flex justify-between mt-2 font-black text-[10px] uppercase italic">
                                ...
                                <div className="p-12 border border-dashed border-white/5 rounded-[2.5rem] text-center text-gray-600 italic text-sm">No hay combates activos en este momento.</div>
                            )
                        ) : (
                            // HISTORIAL DE COMBATES
                            resolvedFights.length > 0 ? resolvedFights.map(fight => {
                                const isAWinner = fight.winner_id === fight.athlete_a_id;
                                const totalPool = fight.pool_a + fight.pool_b;
                                return (
                                    <div key={fight.id} className="bg-[#1a1a1a]/40 border border-white/5 rounded-3xl p-6 relative group overflow-hidden shadow-lg">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">FINALIZADO</span>
                                            <span className="text-[10px] font-black text-yellow-500/80 uppercase">REPARTIDO: {totalPool} AC</span>
                                        </div>
                                        <h4 className="text-sm font-black text-gray-400 uppercase italic mb-4 tracking-widest">{fight.title}</h4>
                                        <div className="flex items-center justify-between gap-4">
                                            <div className={`flex-1 flex items-center gap-3 ${isAWinner ? 'text-green-400' : 'text-gray-700 opacity-40 grayscale'}`}>
                                                <User size={24} />
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-black uppercase italic">{fight.athlete_a?.full_name}</span>
                                                    {isAWinner && <span className="text-[8px] font-bold tracking-tighter">VENCEDOR</span>}
                                                </div>
                                            </div>
                                            <div className="text-gray-800 font-black italic text-xs">VS</div>
                                            <div className={`flex-1 flex items-center gap-3 justify-end text-right ${!isAWinner ? 'text-green-400' : 'text-gray-700 opacity-40 grayscale'}`}>
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-black uppercase italic">{fight.athlete_b?.full_name}</span>
                                                    {!isAWinner && <span className="text-[8px] font-bold tracking-tighter">VENCEDOR</span>}
                                                </div>
                                                <User size={24} />
                                            </div>
                                ...
                                <div className="p-12 border border-dashed border-white/5 rounded-[2.5rem] text-center text-gray-600 italic text-sm">Aún no se ha resuelto ninguna batalla.</div>
                            )
                        )}
                    </div>
                </div>

                {/* PRÓXIMOS EVENTOS (COLORES RESTAURADOS) */}
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        {events.length > 0 ? events.map(event => {
                            const st = getEventStyles(event.name);
                            return (
                                <div key={event.id} className={`group bg-[#1a1a1a]/60 p-6 rounded-2xl border-l-4 ${st.border} border-y border-r border-white/5 hover:bg-[#252525] transition-all shadow-lg`}>
                                    <div className="flex justify-between items-center mb-3">
                                        <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${st.badge}`}>{st.label}</span>
                                    ...
                                    <h4 className="text-2xl font-black text-white uppercase italic group-hover:translate-x-1 transition-transform leading-tight">{event.name}</h4>
            ...
            {/* MODALES: APUESTA, CREACIÓN, RESOLUCIÓN */}
            {betModal.open && betModal.fight && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
                    <div className="bg-[#1a1a1a] border border-white/10 p-8 rounded-[2.5rem] w-full max-w-md relative animate-in zoom-in-95">
                        ...
                        <input type="number" value={betAmount} onChange={(e) => setBetAmount(parseInt(e.target.value))} className="w-full bg-black border border-white/10 p-5 rounded-2xl outline-none font-black text-center text-3xl text-yellow-500 shadow-inner" />
                        <button onClick={handleBet} disabled={isActionLoading} className="w-full py-5 bg-yellow-500 text-black font-black uppercase italic rounded-2xl hover:bg-yellow-400 transition-all mt-8 text-sm shadow-xl tracking-widest">Confirmar Apuesta</button>
                    </div>
                </div>
            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
                    <div className="bg-[#1a1a1a] border border-white/10 p-10 rounded-[2.5rem] w-full max-w-md relative animate-in zoom-in-95">
                        <button onClick={() => { setIsModalOpen(false); setCreationStep('menu'); }} className="absolute top-8 right-8 text-gray-500 hover:text-white"><X size={28} /></button>
                        {creationStep === 'menu' ? (
                            <div className="space-y-4 text-center">
                                ...
                                <div className="p-8 border border-dashed border-white/5 rounded-3xl text-[10px] text-gray-600 font-black uppercase italic opacity-30">Votaciones y Predicciones (Próximamente)</div>
                            </div>
                        ) : (
                            <form onSubmit={handleCreateFight} className="space-y-4">
                                <h2 className="text-2xl font-black uppercase italic text-red-500 mb-6 text-center underline decoration-red-500/20 underline-offset-8">Configurar 1VS1</h2>
                                ...
                                <input required placeholder="ID ATLETA A (UUID DE PERFIL)" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none text-[10px] font-mono" onChange={e => setFightForm({ ...fightForm, athlete_a: e.target.value })} />
                                <input required placeholder="ID ATLETA B (UUID DE PERFIL)" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none text-[10px] font-mono" onChange={e => setFightForm({ ...fightForm, athlete_b: e.target.value })} />
                                <button type="submit" className="w-full py-5 bg-red-600 text-white font-black uppercase italic rounded-2xl hover:bg-red-500 transition-all mt-4 text-xs tracking-widest shadow-lg shadow-red-950/20">Publicar en la Arena</button>
                            </form>
                        )}
                    </div>
                </div>
            ...
            {resolveModal.open && resolveModal.fight && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/98 backdrop-blur-xl">
                    <div className="bg-[#1a1a1a] border border-white/10 p-10 rounded-[3rem] w-full max-w-md text-center shadow-2xl">
                        ...
                        <p className="text-gray-500 text-xs mb-10 font-medium tracking-widest uppercase italic">Bote acumulado: <span className="text-yellow-500 font-black">{resolveModal.fight.pool_a + resolveModal.fight.pool_b} AC</span></p>
                        <div className="grid grid-cols-1 gap-4">
                            <button onClick={() => handleResolveFight('a')} disabled={isActionLoading} className="p-6 bg-red-500/10 border border-red-500/30 rounded-3xl hover:bg-red-500/20 transition-all group">
                                <span className="block text-red-500 font-black uppercase italic text-lg">{resolveModal.fight.athlete_a?.full_name}</span>
                                <span className="text-[10px] text-red-400 font-black tracking-widest uppercase mt-1">Marcar Ganador</span>
                            </button>
                            <button onClick={() => handleResolveFight('b')} disabled={isActionLoading} className="p-6 bg-blue-500/10 border border-blue-500/30 rounded-3xl hover:bg-blue-500/20 transition-all group">
                                <span className="block text-blue-500 font-black uppercase italic text-lg">{resolveModal.fight.athlete_b?.full_name}</span>
                                <span className="text-[10px] text-blue-400 font-black tracking-widest uppercase mt-1">Marcar Ganador</span>
                            </button>
                        </div>
                        <button onClick={() => setResolveModal({ open: false, fight: null })} className="mt-8 text-gray-600 text-[10px] font-black uppercase hover:text-white transition-colors tracking-widest">Volver</button>
                    </div>
                </div>
            )}
        </div>
