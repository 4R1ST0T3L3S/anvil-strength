/*
 * EL ENRUTADO DE anvilstrength.es — LA APLICACIÓN COMPLETA, NO SOLO LA WEB.
 * =====================================================================
 *
 * Esta carpeta se separó el 3 de septiembre de 2026 como "solo
 * promocional" y se le recortó este fichero a seis rutas públicas. Los 384
 * ficheros de la aplicación se quedaron dentro; lo único que faltaba era
 * la puerta.
 *
 * El precio de aquel recorte era doble y se pagaba en el mismo sitio:
 *
 *   - `/auth/callback` SÍ existía, y termina en
 *     `<Navigate to={homeRouteFor(user)}>` → `/dashboard` o
 *     `/coach-dashboard`. Ninguna de las dos estaba enrutada, así que
 *     entrar con Google en anvilstrength.es caía en el comodín `*` y
 *     devolvía a la portada: el login funcionaba y parecía que no.
 *
 *   - Como el panel vivía en OTRO dominio, "entrar" era salir del sitio, y
 *     la sesión no viaja entre orígenes (`localStorage` no se comparte
 *     entre subdominios). Iniciar sesión en la web y luego abrir la app
 *     pedía iniciar sesión otra vez.
 *
 * Con el enrutado completo, `/` sigue siendo la portada para quien no
    ...
    return (
        ...
        <Routes location={location}>
            ...
            {/* --- INVITACIÓN DE UN ENTRENADOR ---
                ...
                Y tienen que estar en ESTA copia y no en otra, porque
                anvilstrength.es es el dominio que el AndroidManifest.xml
                declara con `autoVerify` para los prefijos /reclamar/ e
                /invitacion/: es la dirección que abre la app instalada si la
                hay, y esta web si no la hay. */}
