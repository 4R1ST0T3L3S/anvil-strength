...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      /**
       * DESACTIVADO EN LA VARIANTE ANDROID.
       *
       * Dentro del APK los ficheros ya son locales: precachearlos en el
       * almacenamiento del WebView los duplicaba (3,8 MB) sin ganar nada, y
       * el aviso de "hay una versión nueva" no tiene sentido cuando las
       * versiones llegan por Google Play. Lo que sí sigue cubriendo el
       * entrenamiento sin cobertura son las ESCRITURAS en cola
       * (src/lib/offlineQueue.ts), que no dependen del service worker.
       *
       * Con `disable` los módulos virtuales (`virtual:pwa-register/react`)
       * siguen resolviendo a no-ops, así que nada más cambia. El resto de la
       * configuración se conserva por si esta carpeta vuelve a servirse como
       * web.
       */
      disable: true,
      /**
      ...
      workbox: {
        ...
        // `push-sw.js` (manejadores de Web Push) no existe en esta variante: el APK avisa por Firebase.
