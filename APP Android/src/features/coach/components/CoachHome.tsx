...
function NavTile({
    ...
    return (
        <button
            onClick={onClick}
            className={`group relative flex h-full min-h-[96px] xl:min-h-0 w-full flex-col justify-between overflow-hidden rounded-card border border-[var(--border-default)] bg-surface-raised p-3 text-left transition-colors duration-fast ease-snap hover:bg-surface-overlay active:bg-surface-raised ${customColor.ring}`}
        >
            <Icon
                size={72}
                aria-hidden="true"
