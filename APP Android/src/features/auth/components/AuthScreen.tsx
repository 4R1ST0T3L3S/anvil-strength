...
import { getAuthCallbackUrl, getEmailCallbackUrl } from '../../../lib/authRedirect';
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          // En el APK el navegador lo abrimos nosotros (pestaña personalizada
          // de Chrome) y la vuelta llega por `appUrlOpen`; ver
          // src/hooks/useNativeFeatures.ts. En la web, Supabase redirige solo.
      ...
      if (esNativo() && data?.url) {
        // Si la persona cierra la pestaña sin entrar, el botón deja de girar.
        const oyente = await Browser.addListener('browserFinished', () => {
          setIsGoogleLoading(false);
          void oyente.remove();
        });
        await Browser.open({ url: data.url, toolbarColor: '#0d0f11' });
      }
